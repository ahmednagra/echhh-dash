// src/components/public/PublicCounterBudgetPopup.tsx
'use client';
...
import { PublicPriceNegotiationClientService } from '@/services/public-price-negotiation';
import {PriceNegotiation} from '@/types/price-negotiation';
import { 
...
export default function CounterBudgetPopup({ 
  ...
  const loadNegotiations = async () => {
    ...
    try {
      ...
      const response = await PublicPriceNegotiationClientService.getPublicPriceNegotiations(params);
  ...
  const acceptOffer = async () => {
    ...
    if (!latestNegotiation) {
      ...
      if (!latest) {
        toast.error(`No negotiations found to accept`);
        return;
      }
      toast.error(`No pending offer found. Latest offer: ${latest.proposed_price} ${latest.currency} (${latest.status.name})`);
      return;
    }

    setIsAccepting(true);
    try {
      const acceptRequest: AcceptPublicNegotiationRequest = {
        token
      };

      await PublicPriceNegotiationClientService.acceptPublicNegotiation(latestNegotiation.id, acceptRequest);
      toast.success(`Accepting offer for ${latestNegotiation.proposed_price} ${latestNegotiation.currency}`);
  ...
  const rejectOffer = async () => {
    ...
    if (!latestNegotiation) {
      ...
      if (!latest) {
        toast.error(`No negotiations found to reject`);
        return;
      }
      toast.error(`No pending offer found. Latest offer: ${latest.proposed_price} ${latest.currency} (${latest.status.name})`);
      return;
    ...
    setIsRejecting(true);
    try {
      const rejectRequest: RejectPublicNegotiationRequest = {
        token
      };

      await PublicPriceNegotiationClientService.rejectPublicNegotiation(latestNegotiation.id, rejectRequest);
  ...
  const submitCounter = async () => {
    ...
    if (!latestNegotiation) {
      if (negotiations.length === 0) {
        toast.error(`No negotiations found. Cannot submit counter offer.`);
        return;
      }
      ...
      toast.error(`No pending negotiation found. Latest negotiation status: ${latest.status.name}. Counter offer may not be processed correctly.`);
    }

    setIsSubmitting(true);
    try {
      const targetNegotiation = latestNegotiation || negotiations[0];
      const counterOfferData: CreatePublicCounterOfferRequest = {
      ...
      await PublicPriceNegotiationClientService.createPublicCounterOffer(targetNegotiation.id, counterOfferData);
      toast.error(`Counter offer of ${price} ${currency} submitted`);
      setShowCounterForm(false);
      setPrice('0');
      setNotes('');
      await loadNegotiations();
